...
Create the vault file with the generated sudo password:

```bash
./ansible-vault-setup.sh
```

The sudo password is fetched automatically from `tofu output`. The script only prompts you for a vault passphrase (your own password that encrypts the vault file). This passphrase is saved to `.vault_pass` so Ansible can decrypt the vault automatically on every run.
...
Ansible picks up the vault password automatically via `ansible.cfg`.
